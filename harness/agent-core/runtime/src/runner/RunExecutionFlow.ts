import { createExecutionFlowDefinition, ExecutionFlowInvocation, type ExecutionFlowBasis, type ExecutionFlowContext, type ExecutionFlowStep } from "@agent-anything/observability/execution-flow";
...
const steps = [
  ...
  {id: "admission", label: "Admit Model Calls", kind: "check", checks: ["tool_selection", "tool_input"]},
...
export const RUN_EXECUTION_FLOW = createExecutionFlowDefinition({
  owner: "agent-runtime", id: "run-execution", revision: "1", label: "Harness Core Loop",
...
export class RunExecutionFlow {
  ...
  enter(id: StepId, basis: ExecutionFlowBasis = {}): ExecutionFlowStep {
    const previous = this.current;
    const previousId = this.currentId;
    previous?.end("returned", {}, id);
    const step = this.invocation.enter(id, {basis});
