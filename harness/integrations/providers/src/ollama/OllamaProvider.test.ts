...
describe("OllamaProvider", () => {
  it("observes the existing exchange without extra reads, requests or changed results", async () => {
    const sent: string[] = [];
    const parsed = vi.fn(async () => ({ response: '{"action":"complete","summary":"done"}' }));
    ...
    const baseline = new OllamaProvider(config(), transport);
    const observed = new OllamaProvider(config(), transport, { observe(value) {
  ...
  it("does not report a dispatch or HTTP response for local request rejection", async () => {
    ...
    const observations: import("@agent-anything/model-interaction/transport").ProviderObservation[] = [];
    const provider = new OllamaProvider(config(), transport, { observe(value) { observations.push(value); } });
