...
import type { ValidationSubjectSnapshot, ValidationSubjectSnapshotRef } from "../subject/index.js";
...
export interface ValidationExecutionPort {
  ...
  captureSubject(
    snapshot: ValidationSubjectSnapshot,
    interruption: InvocationInterruptionContext,
  ): Promise<ValidationCurrentSnapshot>;
  executeCheck(
...
export function snapshotCheckAttempt(input: CheckAttempt): CheckAttempt {
  strictRecord(input, "CheckAttempt", [
    ...
    "requestedAt", "startedAt", "interruption", "runAction", "operationInvocation", "actionSettlement",
  ]);
...
export function snapshotCheckResult(input: CheckResult): CheckResult {
  strictRecord(input, "CheckResult", [
    ...
    "finishedAt", "limitations", "failure",
  ]);
  ...
  if (input.status !== "completed" && input.failure === null) {
    throw new TypeError("A non-completed CheckResult requires ValidationFailure.");
  }
  if (input.status === "partial" && input.findings.length === 0) {
    throw new TypeError("A partial CheckResult requires at least one usable Finding.");
  }
