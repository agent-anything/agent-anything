...
import { createVerificationFailure, type VerificationCompletionDisposition } from "../definition/index.js";
import {
...
export class CurrentVerificationCompletionGate implements CompletionGatePort {
  ...
  async evaluate(
    ...
    try {
      ...
      return snapshotCompletionGateDecision({
        ...
        verificationSnapshot: candidate.verificationSnapshot,
        status: "invalid",
        disposition: "fail",
        reasons: [],
        failure: createVerificationFailure({
          code: "verification_gate_input_invalid",
    ...
    if (interruption.signal.aborted) {
      return this.failed(input, "verification_gate_cancelled", "Completion Gate evaluation was cancelled.");
    }
    ...
    if (selected !== null) {
      return this.blocked(input, selected.status, selected.item.disposition!, [{
        owner: "verification",
        code: `verification_requirement_${selected.item.current.status}`,
        message: `A mandatory Verification Requirement is ${selected.item.current.status}.`,
  ...
  private blocked(
    ...
    disposition: VerificationCompletionDisposition,
