...
export function requiredApprovalCategory(
  ...
  switch (prepared.subject.operation.kind) {
    ...
    case "remote_tool": return "remoteToolCall";
...
export function assertApprovalMapping(input: {
  ...
  } else if (expected === "remoteToolCall") {
    if (operation.kind !== "remote_tool") mismatch();
    const typed = payload as ApprovalPayloadByCategory["remoteToolCall"];
    if (
      typed.source.kind !== operation.target.source.kind ||
      typed.source.sourceId !== operation.target.source.sourceId ||
      typed.source.sourceRevision !== operation.target.source.sourceRevision ||
      typed.source.activationEpoch !== operation.target.source.activationEpoch ||
      typed.source.capabilityId !== operation.target.source.capabilityId ||
      typed.server.serverId !== operation.target.server.serverId ||
      typed.server.registrationFingerprint !==
        operation.target.server.registrationFingerprint ||
      typed.server.transport !== operation.target.server.transport ||
      !sameRemoteEndpoint(typed.server.endpoint, operation.target.server.endpoint) ||
      typed.tool.name !== operation.target.toolName
    ) {
      mismatch();
    }
...
function sameRemoteEndpoint(
  left: ApprovalPayloadByCategory["remoteToolCall"]["server"]["endpoint"],
  right: Extract<
    PreparedExternalAction["subject"]["operation"],
  ...
  return (
    ...
    left.applicationProtocol === right.applicationProtocol
  );
}
