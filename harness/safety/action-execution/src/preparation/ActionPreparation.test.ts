...
function createPipeline(overrides: PipelineOverrides = {}): ActionEnforcementPipeline {
  ...
  return new ActionEnforcementPipeline({
    registrations,
    toolBindings: createTestToolBindings(registrations),
    adapters: [{
      actionName: "codeAgent.readFile",
      adapter: createAdapter(
        adapter,
...
function createTestToolBindings(
  registrations: ReturnType<typeof createRegistrations>,
) {
  const tools = createToolRegistrationSnapshot([{
    ...
    source: {
      kind: "harness",
      sourceId: "action-preparation-test",
      sourceRevision: "1",
      activationEpoch: null,
      capabilityId: "codeAgent.readFile",
    },
    schema: { dialect: "json-schema", translationVersion: "1" },
    boundActionName: "codeAgent.readFile",
