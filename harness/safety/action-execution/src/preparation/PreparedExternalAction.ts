...
export interface PreparedActionReference {
  ...
  readonly toolBindingSnapshotId: string;
...
export function createPreparedActionReference(
  ...
  if (
    binding.toolName !== action.name ||
    !binding.origins.includes(action.provenance.origin)
  ) {
    throw contractError(
      "canonical_contract_invalid",
      "Tool Action does not match the selected Tool binding and origin.",
      "action.name",
    );
  }
  return Object.freeze({
    ...
    boundActionName: validateToken(
      binding.boundActionName,
      "binding.boundActionName",
    ),
    toolBindingSnapshotId: validateToken(
      bindingSnapshotId,
      "binding.snapshotId",
      "canonical_token_invalid",
      16_384,
    ),
    toolRegistrationFingerprint: validateToken(
      binding.toolRegistrationFingerprint,
      "binding.toolRegistrationFingerprint",
      "canonical_token_invalid",
      16_384,
    ),
    actionRegistrationFingerprint: validateToken(
...
export function createCanonicalActionSubject(
  ...
  return Object.freeze({
    ...
    toolBinding: Object.freeze({
      snapshotId: input.action.toolBindingSnapshotId,
      toolName: input.action.name,
      toolRegistrationFingerprint: input.action.toolRegistrationFingerprint,
      boundActionName: input.action.boundActionName,
      actionRegistrationFingerprint:
        input.action.actionRegistrationFingerprint,
    }),
...
function assertToolAction(input: Action): void {
  ...
  assertStrictRecord(
    input.provenance,
    "action.provenance",
    new Set(["origin", "modelItemId", "controllerIteration"]),
    "canonical_contract_invalid",
  );
  if (input.provenance.origin !== "model" && input.provenance.origin !== "workflow") {
    throw contractError(
      "canonical_contract_invalid",
      "Action provenance origin must be model or workflow.",
