...
function operationEffectKeys(operation: CanonicalActionOperation): ReadonlySet<string> {
  ...
  switch (operation.kind) {
    ...
    case "network":
      ...
      break;
    case "remote_tool":
      keys.add(`remote_tool:invoke:${canonicalRemoteToolKey(operation.target)}`);
      break;
    case "skill":
      break;
  }
  ...
  for (const effect of effectSet.values) {
    switch (effect.kind) {
      ...
      case "network":
        ...
        break;
      case "remote_tool":
        keys.add(`remote_tool:invoke:${canonicalRemoteToolKey(effect.target)}`);
        break;
    }
  }
  return keys;
...
function collectOperationClaims(
  ...
  switch (operation.kind) {
    ...
    case "network":
      break;
    case "remote_tool":
      addRemoteToolClaim(servers, operation.target);
      break;
    case "skill":
      break;
  }
  ...
  for (const effect of effectSet.values) {
    switch (effect.kind) {
      ...
      case "network":
        break;
      case "remote_tool":
        addRemoteToolClaim(servers, effect.target);
        break;
    }
  }
}
  ...
  for (const assertion of assertions) {
    switch (assertion.kind) {
      ...
      case "remote_tool_identity":
        addRemoteToolClaim(servers, assertion.expected);
        break;
...
function addRemoteToolClaim(
  claims: Map<string, string>,
  target: CanonicalRemoteToolIdentity,
): void {
  addClaim(
    claims,
    canonicalRemoteToolTargetKey(target),
    canonicalRemoteToolKey(target),
    "remote Tool",
  );
}
