...
export function createActionRegistrationSnapshot(
  ...
  registrations.sort((left, right) =>
    left.actionName < right.actionName ? -1 : left.actionName > right.actionName ? 1 : 0
  );
  const frozenRegistrations = Object.freeze(registrations);
  return Object.freeze({
    schemaVersion: 1 as const,
    snapshotId: createSnapshotIdentity(frozenRegistrations),
...
function createSnapshotIdentity(
  ...
  const encoded = registrations
    .map((registration) => encodeToken(registration.registrationFingerprint))
    .join("");
  return `sha256:${createHash("sha256")
    .update("agent-anything.action-registration-snapshot.v1", "utf8")
    .update("\0", "utf8")
    .update(encoded, "utf8")
    .digest("hex")}`;
}
