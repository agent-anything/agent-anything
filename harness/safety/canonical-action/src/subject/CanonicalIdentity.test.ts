...
describe("canonical Action identities", () => {
  ...
  it("retains a positive numeric activation epoch in remote Tool identity", () => {
    const remoteTool = createCanonicalRemoteToolIdentity({
      source: {
        kind: "mcp",
        sourceId: "mcp.local",
        sourceRevision: "revision:1",
        activationEpoch: 2,
        capabilityId: "tools.read",
      },
      server: {
        serverId: "mcp.local",
        registrationFingerprint: "registration:1",
    ...
    expect(remoteTool.source.activationEpoch).toBe(2);
    expect(() => createCanonicalRemoteToolIdentity({
      ...remoteTool,
      source: { ...remoteTool.source, activationEpoch: 0 },
    })).toThrowError(expect.objectContaining({
      code: "canonical_contract_invalid",
      path: "remoteTool.source.activationEpoch",
    }));
  });
