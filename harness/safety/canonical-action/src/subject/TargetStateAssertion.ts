...
export interface RemoteToolIdentityAssertion {
  readonly schemaVersion: 1;
  readonly kind: "remote_tool_identity";
  readonly expected: CanonicalRemoteToolIdentity;
}
...
export type TargetStateAssertionInput =
  ...
  | { readonly kind: "remote_tool_identity"; readonly expected: CanonicalRemoteToolIdentity };
...
export function targetStateAssertionKey(assertion: TargetStateAssertion): string {
  switch (assertion.kind) {
    ...
    case "executor_registration":
      return `${assertion.kind}:${assertion.expected.id}`;
    case "remote_tool_identity":
      return `${assertion.kind}:${canonicalRemoteToolKey(assertion.expected)}`;
  }
}
...
function createAssertion(
  ...
  if (input?.kind === "remote_tool_identity") {
    assertStrictRecord(input, path, new Set(["kind", "expected"]), "canonical_assertion_invalid");
    return Object.freeze({
      schemaVersion: 1,
      kind: "remote_tool_identity",
      expected: createCanonicalRemoteToolIdentity(input.expected),
    });
  }
...
function createExecutorDescriptor(
  ...
  return Object.freeze({
    ...
    physicalPayloadSchemaRevision: validateToken(
