export type CanonicalPathPlatform = "win32" | "posix";
...
export function canonicalizeAbsolutePath(
  ...
  try {
    if (input.length === 0 || input.includes("\0") || /[*?[\]{}]/.test(input)) {
      throw new Error("Filesystem path must be non-empty and contain no null or glob characters.");
    }
    const portable = input.replace(/\\/g, "/");
    const absolute = platform === "posix"
      ? portable.startsWith("/")
      : /^[A-Za-z]:\//.test(portable) || /^\/\/[^/]+\/[^/]+(?:\/|$)/.test(portable);
    if (!absolute) throw new Error(`Path '${input}' must be absolute for ${platform}.`);
    const { prefix, remainder } = splitAbsolutePrefix(portable, platform);
    const segments = normalizeSegments(remainder.split("/"));
    return segments.length === 0
      ? (prefix.endsWith("/") ? prefix : `${prefix}/`)
      : joinPortable(prefix, segments.join("/"));
  } catch (error) {
    throw contractError(
      "canonical_path_invalid",
...
export function canonicalizeConcreteHost(input: unknown, path: string): string {
  ...
  try {
    ...
    const host = new URL(`http://${normalized}`).hostname.toLowerCase();
    if (host.length === 0 || host.split(".").some((label) => label.length === 0)) throw new Error();
    return host;
  } catch {
...
function splitAbsolutePrefix(
  ...
  if (drive) return { prefix: `${drive[1]!.toUpperCase()}:/`, remainder: drive[2] ?? "" };
  const unc = portable.match(/^\/\/([^/]+)\/([^/]+)\/?(.*)$/);
  if (unc) return { prefix: `//${unc[1]}/${unc[2]}`, remainder: unc[3] ?? "" };
  throw new Error(`Path '${portable}' has an invalid absolute prefix.`);
}
...
function normalizeSegments(segments: readonly string[]): string[] {
  ...
  for (const segment of segments) {
    ...
    if (segment === "..") {
      if (normalized.length === 0) throw new Error("Filesystem path escapes its declared root.");
      normalized.pop();
      continue;
    }
