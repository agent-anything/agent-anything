...
export interface CanonicalPathIdentity {
  readonly platform: CanonicalPathPlatform;
...
export interface CanonicalRemoteSourceRef {
...
export function createCanonicalRemoteToolIdentity(
  ...
  assertStrictRecord(
    input,
    "remoteTool",
    new Set(["source", "server", "toolName"]),
    "canonical_contract_invalid",
  );
  const source = snapshotCanonicalRemoteSourceRef(input.source);
  if (source.kind !== "mcp" && source.kind !== "plugin" && source.kind !== "remote") {
    throw contractError(
      "canonical_contract_invalid",
      "A remote Tool requires MCP, Plugin, or remote source provenance.",
      "remoteTool.source.kind",
    );
  }
  return Object.freeze({
    source: source as CanonicalRemoteToolIdentity["source"],
...
export function canonicalRemoteToolKey(target: CanonicalRemoteToolIdentity): string {
  return `${canonicalToolSourceKey(target.source)}:${target.server.serverId}:${
    target.server.registrationFingerprint
  }:${target.toolName}`;
}
...
function canonicalToolSourceKey(source: CanonicalRemoteSourceRef): string {
  return `${source.kind}:${source.sourceId}:${source.sourceRevision ?? ""}:${
    source.activationEpoch ?? ""
  }:${source.capabilityId}`;
}

function snapshotCanonicalRemoteSourceRef(
  input: CanonicalRemoteSourceRef,
): CanonicalRemoteSourceRef {
  assertStrictRecord(
    input,
    "remoteTool.source",
    new Set(["kind", "sourceId", "sourceRevision", "activationEpoch", "capabilityId"]),
    "canonical_contract_invalid",
  );
  if (input.kind !== "mcp" && input.kind !== "plugin" && input.kind !== "remote") {
    throw contractError("canonical_contract_invalid", "Invalid remote source kind.", "remoteTool.source.kind");
  }
  return Object.freeze({
    kind: input.kind,
    sourceId: validateToken(input.sourceId, "remoteTool.source.sourceId"),
    sourceRevision: input.sourceRevision === null
      ? null
      : validateToken(input.sourceRevision, "remoteTool.source.sourceRevision"),
    activationEpoch: input.activationEpoch === null
      ? null
      : validatePositiveSafeInteger(
          input.activationEpoch,
          "remoteTool.source.activationEpoch",
        ),
    capabilityId: validateToken(input.capabilityId, "remoteTool.source.capabilityId"),
  });
}

function validatePositiveSafeInteger(input: unknown, path: string): number {
  if (
    typeof input !== "number" ||
