...
export function ExecutionFlowView({scope,runId,paused=false,params,navigate,onSubject,onRecord,onContent}: Props) {
  ...
  const graph = <CanvasFrame title="Execution Flow"><div className="flow-graph-panel"><Segmented aria-label="Flow graph mode" options={["Definition","Occurrence"]} value={graphMode} onChange={setGraphMode}/><div className="flow-graph-body">{graphMode === "Definition" ? definitionGraph : occurrenceId ? <Suspense fallback={<Spin/>}><RelationGraph graph={neighborhood} selected={entry?.subject ?? null} onSelect={subject=>{void follow(subject);}} onLink={id=>{const link=links.find(link=>link.id===id);if(link)onRecord(link.establishedBy);}}/></Suspense> : <Empty description="Select an occurrence to inspect its recorded neighbors"/>}</div></div></CanvasFrame>;
  ...
  return <div className="execution-flow-view" aria-busy={busy || paused}><div className="view-toolbar"><Tooltip title="Back to recorded caller"><Button aria-label="Back to caller" disabled={!caller} icon={<ArrowLeftOutlined />} onClick={() => {if(caller) void follow(caller.subject);}} /></Tooltip><Select aria-label="Flow invocation" showSearch optionFilterProp="label" value={bundle?.invocationId ?? undefined} placeholder="Recorded invocation" options={bundle?.catalog.records.flatMap(record => {const fact=entered(record);return fact ? [{value:fact.invocationId,label:`${fact.definition.owner} / ${fact.definition.id} / #${record.commitSequence}`}] : [];}) ?? []} onChange={value => navigateTo(value)} />{busy && <Spin size="small" />}{bundle?.catalog.next && <Button size="small" loading={paging} onClick={() => {void more("catalog");}}>More invocations</Button>}</div>
    ...
    <div className={`flow-workspace${desktop ? "" : " flow-compact"}`}>
      {!desktop && <Segmented aria-label="Execution flow panel" options={["Graph","Occurrences","Details"]} value={pane} onChange={setPane} />}
      <div className={`flow-panels flow-pane-${pane.toLowerCase()}`}>
        <CanvasSplit view="execution-flow" canvas={graph} details={<Splitter className="flow-support-split"><Splitter.Panel defaultSize="45%" min={210}>{occurrences}</Splitter.Panel><Splitter.Panel min={250}>{details}</Splitter.Panel></Splitter>} />
      </div>
    </div>
  </div>;
}
