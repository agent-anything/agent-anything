...
test("execution flow retains exact occurrences, child navigation and manual refresh", async ({page}, testInfo) => {
  ...
  try {
    ...
    await expect(page.locator('.flow-graph[data-layout="ready"]')).toBeVisible();
    const chart = await page.locator(".flow-graph .react-flow").elementHandle();
    const viewport = page.locator(".flow-graph .react-flow__viewport");
    const transform = await viewport.getAttribute("style");
    await page.getByRole("button",{name:"Expand Execution Flow canvas",exact:true}).click();
    ...
    await page.getByRole("button",{name:"Restore Execution Flow canvas",exact:true}).click();
