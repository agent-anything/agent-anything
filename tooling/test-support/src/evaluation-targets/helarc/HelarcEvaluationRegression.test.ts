...
describe("Helarc accepted Evaluation baseline succession", () => {
  it("preserves accepted history and proves the Delegation Transfer baseline successor", async () => {
    const baselines = [
    ...
    expect(candidate.report.ref).toEqual(HELARC_DELEGATION_TRANSFER_ACCEPTED_BASELINE.reportRef);
    expect(candidate.acceptance.ref).toEqual(HELARC_DELEGATION_TRANSFER_ACCEPTED_BASELINE.acceptanceRef);
    ...
    expect(candidate.metrics.map(({ ref }) => ref)).toEqual(
      HELARC_DELEGATION_TRANSFER_ACCEPTED_BASELINE.metrics.map(({ ref }) => ref),
    );
  ...
  it("reports a safety regression even when paired latency improves", () => {
    ...
    const latencyIndex = HELARC_DELEGATION_TRANSFER_ACCEPTED_BASELINE.metrics.findIndex(
