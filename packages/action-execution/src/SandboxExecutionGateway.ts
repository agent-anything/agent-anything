  ...
  private async dispatchDisabled(input: {
    ...
    try {
      ...
      if (result?.status === "failed") {
        return executorFailed(input.attempt, snapshotExecutorFailure(result.failure));
      }
...
function snapshotToolResult(
  ...
  try {
    ...
    throw new ToolResultContractError("ActionExecutor ToolResult is not serializable.");
  }
...
function assertExactToolResultShape(result: ToolResult): void {
  if (
    ...
    throw new ToolResultContractError("ActionExecutor ToolResult must be a plain object.");
  }
  const common = [
    "toolCallId",
    "toolName",
  ...
  switch (result.status) {
    case "succeeded":
      assertExactDataProperties(result, [...common, "output"], "Succeeded ToolResult");
      if (result.output === null || result.output === undefined) {
        throw new ToolResultContractError("Succeeded ToolResult requires output.");
      }
      return;
    case "partial":
    ...
    case "failed":
    case "timeout":
      assertExactDataProperties(result, [...common, "error"], "Failed ToolResult");
      assertToolResultError(result.error);
      return;
    default:
      throw new ToolResultContractError(
...
function assertToolResultError(error: unknown): void {
  if (
    ...
    throw new ToolResultContractError("ToolResult error must be a plain object.");
  }
  assertExactDataProperties(
    error,
    "metadata" in error ? ["code", "message", "metadata"] : ["code", "message"],
    "ToolResult error",
  );
...
function assertExactDataProperties(
  ...
  for (const key of keys) {
    if (typeof key !== "string" || !expected.has(key)) {
      throw new ToolResultContractError(`${label} contains an unsupported field.`);
    }
    ...
    if (
      ...
      throw new ToolResultContractError(`${label} contains an invalid property.`);
    }
  }
}

function snapshotExecutorFailure(failure: ActionExecutorFailure): ActionExecutorFailure {
  if (
    failure === null ||
  ...
  if (
    ...
    throw new ActionExecutorResultContractError("ActionExecutor failure is invalid.");
  }
