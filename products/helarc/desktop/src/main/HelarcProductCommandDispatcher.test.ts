...
describe("Helarc Product command dispatcher", () => {
  it("validates exact envelopes and route kinds before invoking an owner", async () => {
    ...
    await expect(dispatcher.dispatch({
      ...command("run.start", "command-version", {
    ...
    await expect(dispatcher.dispatch({
      ...command("run.start", "command-extra", {
        taskText: "Inspect files",
        target: { kind: "new_thread" },
      }),
      privateControl: true,
    ...
    await expect(dispatcher.dispatch(
      command("run.start", "command-route", {
        taskText: "Inspect files",
        target: { kind: "new_thread" },
      }),
      "thread.open",
    ...
    await expect(dispatcher.dispatch({
      ...
      commandId: "command-target-missing",
      kind: "run.start",
    ...
    await expect(dispatcher.dispatch({
      version: HELARC_PRODUCT_COMMAND_VERSION,
      commandId: "command-target-stale-shape",
      kind: "run.start",
      payload: {
        taskText: "Inspect files",
        target: {
