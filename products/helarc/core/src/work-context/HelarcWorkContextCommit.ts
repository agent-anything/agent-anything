...
import type { HelarcArtifact } from "../artifacts/HelarcArtifact.js";
...
export async function applyHelarcRunStartCommit(
  ...
  if (commit.run.id !== commit.runId || commit.run.threadId !== commit.threadId ||
    ...
    commit.run.triggeringThreadRevision !== commit.expectedThreadRevision ||
  ...
  if (commit.target.kind === "create_thread") {
    ...
    record = {
      thread: {
        ...thread,
        revision: commit.expectedThreadRevision + 1,
...
> {
  if (
    ...
    !Number.isSafeInteger(commit.expectedThreadRevision) || commit.expectedThreadRevision < 0
  ) {
  ...
  const currentRevision = aggregate?.record.thread.revision ?? 0;
  if (commit.expectedThreadRevision !== currentRevision) {
    return {
      status: "settled",
      result: reject(
        aggregate,
        "stale_thread_revision",
        "Commit expected Thread revision is stale.",
      ),
    };
  }
...
function isValidAssistantMessage(
  ...
  return message.role === "assistant" && message.threadId === commit.threadId &&
    message.sequence === record.messages.length + 1 &&
    message.source.kind === "agent_run" &&
    message.correlation.runId === commit.runId &&
...
function sameWorkspaceSelectionSelection(
...
function isLedgerConsistentWithRecord(
  ...
  if (
    ledger.length === 0 || ledger.at(-1)?.committedAt !== record.thread.updatedAt ||
    ledger.at(-1)?.committedThreadRevision !== record.thread.revision
  ) {
    return false;
  }
  ...
  let previousThreadRevision = 0;
  for (const entry of ledger) {
    if (!runById.has(entry.runId) ||
      entry.expectedThreadRevision !== previousThreadRevision ||
    ...
    previousThreadRevision = entry.committedThreadRevision;
