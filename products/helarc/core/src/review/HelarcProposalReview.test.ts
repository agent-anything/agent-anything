...
function createProposal(operation: PatchOperation): PatchProposal {
  return {
    ...
    operation,
    sourceSnapshot: {
      target: { rootName: "code", workspaceId: "workspace-1", path: operation.path },
      baseline: { kind: "absent" },
      content: null,
      contentRef: null,
      capturedAt: "2026-06-20T10:00:00.000Z",
    },
    producer: { kind: "controller", owner: "helarc", refId: "model-item-1" },
    creationBasis: { kind: "controller_output", refId: "model-item-1" },
    sensitivity: "private",
...
describe("PatchContracts", () => {
  ...
  it("represents proposal and review-decision lifecycle states", () => {
    ...
    const revisionRequested: RevisionRequestedPatchDecision = {
      ...
      decidedAt: "2026-06-20T10:01:00.000Z",
      reason: "Revise the target content.",
      metadata: {},
    };
    const states: PatchStatus[] = [
      ...
      { status: "revision_requested", proposal, decision: revisionRequested },
    ];
