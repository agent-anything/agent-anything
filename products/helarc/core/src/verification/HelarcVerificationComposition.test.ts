...
} from "./HelarcVerificationComposition.js";
import {
...
describe("Helarc Verification composition", () => {
  it("materializes the single Code Agent profile with command Verification", async () => {
...
async function createComposition(
  ...
  return createHelarcVerificationComposition({
    workspace: WORKSPACE,
    codeSource,
...
async function prepare(
  ...
  operationChecks: VerificationOperationCheckResolverPort = Object.freeze({ resolve: () => null }),
): Promise<VerificationExecutionPort> {
  const execution = await composition.runner.executionFactory.create({ run: RUN, operationChecks });
  const materialized = materializeVerificationProfile({ profile: composition.profile, run: RUN, createdAt: NOW });
...
function owner(id: string, kind: string): VerificationOwnerRef {
