...
export async function inspectPreparedFileSystemTarget(input: {
  ...
  readonly canonicalTarget: string;
  readonly path: string;
}): Promise<{ readonly pathIdentity: CanonicalPathIdentity; readonly baseline: FileBaseline }> {
  ...
  return Object.freeze({
    pathIdentity: createCanonicalPathIdentity({
      platform: input.platform,
      path: isWorkspaceRootTarget
...
export function sameCanonicalPathIdentity(
  left: CanonicalPathIdentity,
  right: CanonicalPathIdentity,
): boolean {
  return canonicalPathIdentityKey(left) === canonicalPathIdentityKey(right);
}

export function sameFileBaseline(
  left: FileBaseline,
  right: FileBaseline,
): boolean {
